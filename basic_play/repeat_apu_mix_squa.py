...
##写入wav流
def write_note(time, framerate, file, sampwidth, freq1, vol1, duty1, freq2, vol2, duty2):
    t = 0   #时刻
    #占空比
    if duty1 == 0 :
        PCT1 = 12.5
...
SAMPWIDTH = 2
linecnt = 0
wf = wave.open('APU_MIX_SUQA.wav', 'w')
...
while(runnable != 0) :
    ...
    print('cnt=' + str(cnt) + ' ||squa1: ' + str(wave_len1) + ' ' + str(note_freq1) + ' ' + str(vol1) + ' ' + str(duty1) \
           + ' ||squa2: ' + str(wave_len2) + ' ' + str(note_freq2) + ' ' + str(vol2) + ' ' + str(duty2))
    write_note(frame_period, FRAMERATE, wf, SAMPWIDTH, note_freq1, vol1, duty1, note_freq2, vol2, duty2)
    end_time = time.time()
...
print('开始播放wav')
CHUNK = 1
wf = wave.open('APU_MIX_SUQA.wav', 'rb')
